...
func _get_drag_data(_at_position: Vector2):
	if item_origin_pointer != -1:
		...
		previewScene.node.position = Vector2(-(real_size.x/2)+relIndex.x*real_size.x, -(real_size.x/2)*real_size.y+relIndex.y*real_size.y)
...
func _can_drop_data(_at_position: Vector2, data) -> bool:
	if not preview_slot:
		...
		preview_slot = load("res://src/entities/types/overlay/inventory/preview.tscn").instantiate()
		preview_slot.node.add_child(preview)
		preview_slot.global_position = global_position-Vector2(data["offset"])*Vector2(real_size)
		add_child(preview_slot)

		can_add = root.can_add(preview.geometry, root.coords_to_index(root.index_to_coords(root.slots.find(self))-Vector2i(data["offset"])), preview.rotate, data["path"])
		
		if can_add:
			preview.modulate = Color("ffffff8f")
		else:
			preview.modulate = Color("c30016b8")
	return can_add
